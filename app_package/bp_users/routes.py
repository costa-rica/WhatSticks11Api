...
from ws_models import DatabaseSession, Users, OuraToken, OuraSleepDescriptions, AppleHealthQuantityCategory, \
...
def login():
    ...
    if auth.password:
        ...
        if bcrypt.checkpw(auth.password.encode(), user.password.encode()):
            ...
            oura_token_obj = db_session.query(OuraToken).filter_by(user_id=user.id).first()
...
def update_user_location_with_lat_lon(current_user):
    ...
    logger_bp_users.info(f"**** ************************ ******")
    logger_bp_users.info(f"**** changed current user and removed sess.commit() ******")
    logger_bp_users.info(f"**** committing new location_reoccuring_permission ******")
    logger_bp_users.info(f"**** ************************ ******")
